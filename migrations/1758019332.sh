...
collectiveos-refresh-config uwsm/default
collectiveos-refresh-config uwsm/env
collectiveos-state set reboot-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, collectiveos-launch-editor' ~/.config/hypr/bindings.conf
fi
