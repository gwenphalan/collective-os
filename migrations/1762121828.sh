...
# https://github.com/basecamp/collectiveos/issues/1852
...
collectiveos-pkg-add xdg-terminal-exec
...
if command -v alacritty > /dev/null 2>&1; then
  cp "$COLLECTIVEOS_PATH/applications/Alacritty.desktop" ~/.local/share/applications/
fi
...
if [ -f "$waybar_config" ]; then
  sed -i 's|"on-click-right": "collectiveos-launch-terminal"|"on-click-right": "xdg-terminal-exec"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL -e btop"|"on-click": "xdg-terminal-exec btop"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL --class=Wiremix -e wiremix"|"on-click": "xdg-terminal-exec --app-id=com.collectiveos.Wiremix -e wiremix"|' "$waybar_config"
...
if [ -f "$system_conf" ]; then
  if grep -q 'class:(.*|Impala|' "$system_conf" || grep -q 'class:(.*|Wiremix|' "$system_conf" || grep -q '|CollectiveOS|' "$system_conf"; then
    sed -i 's/\bImpala\b/com.collectiveos.Impala/g; s/\bWiremix\b/com.collectiveos.Wiremix/g; s/|CollectiveOS|/|com.collectiveos.CollectiveOS|/g' "$system_conf"
  fi
fi
