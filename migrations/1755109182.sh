...
# Users can still opt-in to Cloudflare DNS using: collectiveos-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-collectiveos-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-collectiveos-dns.network
  ...
  echo "To use Cloudflare DNS, run: collectiveos-setup-dns Cloudflare"
fi